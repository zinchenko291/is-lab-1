package me.zinch.is.islab1jee8.models.dao.interfaces;

import me.zinch.is.islab1jee8.models.entities.FuelType;
import me.zinch.is.islab1jee8.models.entities.Vehicle;
import me.zinch.is.islab1jee8.models.fields.Filter;
import me.zinch.is.islab1jee8.models.fields.Range;
import me.zinch.is.islab1jee8.models.fields.VehicleField;

import java.util.List;
import java.util.Optional;

public interface IVehicleDao extends IDao<Vehicle, VehicleField> {
    Optional<Vehicle> findMinEnginePower();
    Long countGtFuelType(FuelType fuelType);
    List<Vehicle> findByNameSubstring(Integer page, Integer pageSize, Filter<VehicleField> filter, String name);
    List<Vehicle> findByEnginePowerRange(Range<Integer> range);
    Optional<Vehicle> resetDistanceTravelledById(Integer id);
}
