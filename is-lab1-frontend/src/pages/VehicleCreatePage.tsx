...
import type { VehicleWithoutIdDto, FuelType } from '../api/models/vehicles';
import type {
  CoordinateMode,
  FormErrors,
  FormState,
} from '../components/createVehicle/vehicleFormTypes';
import validateVehicleForm from '../api/validators/validateVehicleForm';
import { toaster } from '../components/ui/toaster';
import VehicleForm from '../components/createVehicle/VehicleForm';
import CoordinatesListPanel from '../components/createVehicle/CoordinatesListPanel';
...
const VehicleCreatePage = observer(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors((prev) => ({
        ...prev,
        common: err?.message ?? 'Не удалось создать транспортное средство',
      }));
    } finally {
      setSubmitting(false);
    }
